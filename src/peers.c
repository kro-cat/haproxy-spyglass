 ...
 *
 * Peers protocol communication functions.
 */
...
#define PROTOCOL_IDENT HAProxyS
#define PROTOCOL_VERSION 2.1

struct f_hello {
	const char *const remote_peer;
	const char *const local_peer;
	int pid;
	int rel_pid;
};
int ms_hello(message_t message, void *frame);
int mr_status(message_t message, void *frame);
 ...
 */
int do_handshake(const char *const remote_peer, const char *const local_peer,
		 int pid, int rel_pid)
{
	/*
	...
	struct f_hello f_hello = { remote_peer, local_peer, pid, rel_pid };
	message_attr_t hello_mattr = { ms_hello, NULL, &f_hello };

	send_message(hello_mattr);

	message_t status = recv_message();
	destroy_message(status, NULL);
}
...
int ms_hello(message_t message, void *frame)
{
	struct f_hello *pf_hello = (struct f_hello *)frame;

	int length = asprintf(&(message->data),
			      "" #PROTOCOL_IDENT " " #PROTOCOL_VERSION "\n"
			      "%s\n"
			      "%s %d %d\n",
			      pf_hello->remote_peer, pf_hello->local_peer,
			      pf_hello->pid, pf_hello->rel_pid);
